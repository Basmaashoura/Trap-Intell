...
internal sealed class PauseHoneypotCommandHandler : IRequestHandler<PauseHoneypotCommand, Result>
{
    private readonly IHoneypotRepository _honeypotRepository;
    private readonly IUnitOfWork _unitOfWork;

    public PauseHoneypotCommandHandler(
    ...
    public async Task<Result> Handle(PauseHoneypotCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = honeypot.Pause(request.Reason);
        if (result.IsFailure)
            return result;
